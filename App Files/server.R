...
#------------------------------------------
 ...
 data.file <- reactive({
		...
		if(!is.null(inFile)){
			...
			# NEED TO FIGURE OUT FILE FORMAT DETAILS!
			#data.file.tmp <- read.csv(inFile$datapath, stringsAsFactors=FALSE)
			#data.file.tmp <- read.csv(inFile$datapath, fileEncoding="UTF-8-BOM", stringsAsFactors = FALSE)
			data.use <- read_csv(inFile$datapath)

			#yrs.window <- min(data.use$Year) : max(data.use$Year) # need this for all the other tabs -> just keep all the records


			# May need something like this, keep here as reminder
...
data.selected <- reactive({
    ...
    data.sub <- data.sub %>% dplyr::filter(DU==input$selected.du) %>%
    							mutate(Abd = Abd/input$abd.rescale)
...
du.list <-  reactive({data.file.tmp <- data.file(); return(sort(unique(data.file.tmp$DU)))})

output$du.list.menu <- renderUI({
			selectInput("selected.du", label = "Select a DU", choices = du.list(),
								multiple=FALSE,selected = du.list()[1] )
	})
...
output$plot.full.series<- renderPlot({

	data.file.tmp <- data.selected()
